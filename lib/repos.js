...
var getGithubRepo(owner, repoName, cb) {
...
var log = function(repo, limit, cb) {
  if (_.isFunction(limit)) {
    cb = limit;
    limit = 20;
  }
  repo.commits("master", limit, function(err, data) {
    if (err) return cb(err);
    cb(null, _.sortByOrder(data, ['committed_date:'], [false]));
  });
};
